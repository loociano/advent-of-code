...
def load_reactions(reactions: Sequence[str]) -> dict:
  formulas = {}
  for reaction in reactions:
    inputs, output = reaction.rstrip().split('=>')
...
def part_one(reactions: Sequence[str], fuel_amount: int = 1) -> int:
  formulas = load_reactions(reactions)
  totals = {}
...
def part_two(reactions: Sequence[str]):
  formulas = load_reactions(reactions)
  totals = {}
  for key in formulas.keys():
    totals[key] = 0
