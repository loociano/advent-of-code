...
def _parse_input(integers: str) -> Sequence[int]:
  return tuple(map(int, integers))


def fft(digits: Sequence[int], target_phases: int) -> list:
  output = list(digits)
  base_pat = [0, 1, 0, -1]
...
def part_one(integers: str, target_phases: int) -> str:
  output = fft(_parse_input(integers), target_phases)
  return ''.join(map(str, output[:8]))


def part_two(integers: str, target_phases: int) -> str:
  input_num = _parse_input(integers) * 10000
