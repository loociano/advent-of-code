...
def find_start(grid: Grid) -> (int, int):
...
def find_portals(grid: Grid) -> (dict, dict):
  portals_key_pos = {}
  portals_key_name = {}
...
def warp_to(grid: Grid, x: int, y: int) -> (int, int):
  for dx_y in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
    if grid[y + dx_y[1]][x + dx_y[0]] == '.':
      return x + dx_y[0], y + dx_y[1]


def within_bounds(grid: Grid, pos: (int, int)) -> bool:
  return 0 <= pos[0] < len(grid[0]) and 0 <= pos[1] < len(grid) and grid[pos[1]][pos[0]] != '#'


def bsf_min_steps(grid: Grid, portals_key_pos: dict, portals_key_name: dict, start: (int, int), recursive=False) -> int:
  q = [(start[0], start[1], 0)]
  visited = set()
  steps = -1
  while len(q) > 0:
