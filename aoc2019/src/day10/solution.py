...
def _get_asteroid_grid(asteroid_grid: Sequence[str]) -> Grid:
  return tuple(tuple(line) for line in asteroid_grid)


def _find_asteroid_pos(asteroid_map: Grid, width: int, height: int, x: int, y: int, dx: int, dy: int) -> tuple or None:
  pos_x = x + dx
  pos_y = y + dy
...
def _get_monitoring_station_position(asteroid_grid: Grid):
...
def part_one(asteroid_grid: Sequence[str]) -> int:
  return _get_monitoring_station_position(_get_asteroid_grid(asteroid_grid))[0]


def part_two(asteroid_grid: Sequence[str], vaporized_target: int) -> int:
  asteroid_grid = _get_asteroid_grid(asteroid_grid)
  asteroid_grid = list(list(line) for line in asteroid_grid)
  width, height = len(asteroid_grid[0]), len(asteroid_grid)
  vectors = _calc_vectors(width, height)
