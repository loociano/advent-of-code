...
def _get_wires(wires: Sequence[str]) -> (list, list):
  return tuple(wires[0].split(',')), tuple(wires[1].split(','))
...
def part_one(wires: Sequence[str]) -> int:
  wire1, wire2 = _get_wires(wires)
  area_dict = {}
  delay_dict = {}
...
def part_two(wires: Sequence[str]) -> int:
  wire1, wire2 = _get_wires(wires)
  area_dict = {}
  delay_dict = {}
